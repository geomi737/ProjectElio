...
class Trainer:
    ...
    def __post_init__(self):
        ...
        self.scheduler = torch.optim.lr_scheduler.OneCycleLR(self.optimizer,
                                                             self.learning_rate,
                                                             total_steps=self.epochs * (len(self.training_dataloader) // self.accumulation),
                                                             pct_start=self.pct_start)
        self.scaler = torch.GradScaler(device)

    def calculate_loss(self, loss_type):
        losses_list = self.losses if not loss_type else self.val_losses
        if not losses_list:
            return 0.0
        total = sum(l.item() if isinstance(l, torch.Tensor) else l for l in losses_list)
        result = total / len(losses_list)
        losses_list.clear()
        return result

    def get_graph(self):
        train_val = sum(l.item() if isinstance(l, torch.Tensor) else l for l in self.global_train_losses) / \
            max(1, len(self.global_train_losses))
        val_val = sum(l.item() if isinstance(l, torch.Tensor) else l for l in self.global_val_losses) / \
            max(1, len(self.global_val_losses))
        self.mean_train_losses.append(train_val)
        self.mean_val_losses.append(val_val)
    ...
    def create_backup(self, epoch, step):
        try:
            ...
            checkpoint = {
                ...
                "global_train_losses": [l.item() if isinstance(l, torch.Tensor) else l for l in self.global_train_losses],
                "global_val_losses": [l.item() if isinstance(l, torch.Tensor) else l for l in self.global_val_losses],
            ...
            for epoch in range(self.start_epoch, self.epochs + 1):
                ...
                pbar = tqdm.tqdm(
                    enumerate(
                        ...
                        1),
                    desc=f"Training / Epoch: {epoch} / Loss: NaN",
                    total=len(
                        self.training_dataloader))
                self.model.train()
                for step, idxtrg in pbar:
                    ...
                    idx, target = idxtrg
                    idx, target = idx.long().to(device, non_blocking=True), target.long().to(device, non_blocking=True)
                    ...
                    self.global_train_losses.append(loss_detached)
                    self.losses.append(loss_detached)

                    scaled_loss = loss / self.accumulation
                    ...
                    if step % (self.accumulation * 10) == 0:
                ...
                pbar = tqdm.tqdm(
                    enumerate(
                        ...
                        1),
                    desc=f"Validating / Epoch: {epoch} / Loss: NaN",
                    total=len(
                        self.validation_dataloader))
                for step, valtrg in pbar:
                    val, target = valtrg
                    val, target = val.long().to(device, non_blocking=True), target.long().to(device, non_blocking=True)
                    ...
                    loss_detached = loss.detach()
                    self.global_val_losses.append(loss_detached)
                    self.val_losses.append(loss_detached)
